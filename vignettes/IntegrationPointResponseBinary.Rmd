...
Your R script could contain a function such as this one, with a name of your choice. All applicable input variables must be declared, even if they are not used in the script. Input variables that are not applicable (depending on standard binary outcome vs. Vaccine Efficacy) must not be declared. We recommend always declaring `UserParam` as a default `NULL` value in the function arguments, as this will ensure that the same function will work regardless of whether the user has specified any custom parameters in East Horizon. For Vaccine Efficacy, `OneMinusROP` must be declared with default value `NULL`.
...
```
GenerateResponse <- function( NumSub, NumArm, TreatmentID, FollowUpDur, PropResp, OneMinusROP = NULL, UserParam = NULL )
{
  nError        	<- 0 # Error handling (no error)
  vPatientOutcome 	<- rep( 0, NumSub ) # Initializing response array to 0	
...
2. [**Multiple Arm, Patient Simulation**](MultiArmOutcomePatientSimulation.html)
    - [SimulatePatientOutcomePercentAtZero.Binary.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/MultiArmOutcomePatientSimulation/Binary/R/SimulatePatientOutcomePercentAtZero.Binary.R)
