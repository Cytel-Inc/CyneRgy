...
|**AnalysisTime**|Numeric|Optional. Estimate of analysis time. For interim analyses, equivalent to look time; for final analysis, equivalent to study duration.|
|**OutList**|List|Optional. List of outputs to pass to the next look. Only relevant for `Statistical Design = Group Sequential or Group Sequential with Sample Size Re-Estimation`. Will be available as input to this function in the next look. See above in the Input Variables.|
|**POCStatusArm**|Vector of Numeric|Optional. Vector of length `NumArm`, indicating whether the Proof of Concept threshold has been crossed in the current look for each treatment arm:<br>- `0`: Has not been crossed.<br>- `1`: Has been crossed. Only relevant for `Study Design = Dose Finding`.|
|**POCStatus**|Numeric|Optional. Indicates whether the Proof of Concept threshold has been crossed in the current look:<br>- `0`: Has not been crossed.<br>- `1`: Has been crossed. Only relevant for `Study Design = Dose Finding`.|
| **ErrorCode**   | Integer  | Optional. Can be used to handle errors in your script:<br>– `0`: No error.<br> – `Positive Integer`: Nonfatal error, the current simulation will be aborted, but the next simulation will proceed.<br>– `Negative Integer`: Fatal error, no further simulations will be attempted.                  |
...
Detailed templates with step-by-step explanations are available here: [Analyze.Continuous.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.Continuous.R) for 2-Arm, [Analyze.Continuous.SSR.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.Continuous.SSR.R) for 2-Arm with SSR, [Analyze.Continuous.MAMS.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.Continuous.MAMS.R) for Multiple Arm, and [Analyze.DoseFinding.Continuous.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.DoseFinding.Continuous.R) for Dose Finding.
...
3. [**Dose Finding, Analysis**](DoseFindingAnalysis.html)
