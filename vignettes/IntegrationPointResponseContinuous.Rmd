...
Your R script could contain a function such as this one, with a name of your choice. All applicable input variables must be declared, even if they are not used in the script. Input variables that are not applicable (depending on the study objective) must not be declared. Optional parameters such as `ArrivalTime` may be omitted. We recommend always declaring `UserParam` as a default `NULL` value in the function arguments, as this will ensure that the same function will work regardless of whether the user has specified any custom parameters in East Horizon.

Detailed templates with step-by-step explanations are available here: [SimulatePatientOutcome.Continuous.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/SimulatePatientOutcome.Continuous.R) for 2-Arm and [SimulatePatientOutcome.Continuous.MAMS.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/SimulatePatientOutcome.Continuous.MAMS.R) for Multiple Arm.
...
1. [**2-Arm, Continuous Outcome - Patient Simulation**](2ArmNormalOutcomePatientSimulation.html)
...
2. [**Childhood Anxiety Trial**](ChildhoodAnxiety.html)
...
3. [**Multiple Arm, Patient Simulation**](MultiArmPatientSimulation.html)
