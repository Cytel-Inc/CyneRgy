---
title: "Integration Point: Dropout - Dual Endpoints (TTE-TTE or TTE-Binary)"
...
|**PrdTime**|Vector of Numeric|Vector of length `NumPrd`, indicating the times for dropout parameters. Depends on `DropMethod`:<br>– If `DropMethod = 1`: Starting times of dropout periods.<br>– If `DropMethod = 2`: Times at which the probabilities of dropout are specified.|
...
```
Explore the example below for more context. Note that the example illustrates a single-endpoint setup; however, the same configuration applies to dual-endpoint projects.

1. [**2-Arm, Single Endpoint - Simulate Patient Dropout**](2ArmPatientDropout.html)
