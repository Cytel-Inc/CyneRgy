...
|----------------|--------------|---------------------------------------------------------------------------------|
| **Decision**   | Integer  | Boundary crossing decision:<br>– `0`: No boundary crossed.<br>– `1`: Lower efficacy boundary crossed.<br>– `2`: Upper efficacy boundary crossed.<br>– `3`: Futility boundary crossed (only applicable when `Statistical Design = Group Sequential`).<br>– `4`: Equivalence boundary crossed (not available in East Horizon Explore).<br>You can use the functions `CyneRgy::GetDecisionString` and `CyneRgy::GetDecision` to get the decision value. See the template below for the correct usage. If `Subpopulations` is turned on, this is a named list of integer of length `DesignParam$NumSubPops`, with decisions for each subpopulation, e.g., `Decision[“FullPopulation”]`, `Decision[“SubPopulation 1”]`, etc.|
...
|**ReEstEvents**|Integer|Re-estimated number of events. Only required for `Statistical Design = Group Sequential with Sample Size Re-Estimation`.|
...
**Note for `Statistical Design = Group Sequential with Sample Size Re-Estimation`**: *ReEstEvents* is required. Only Option 1 is supported, all other output options are not available.

**Note for `Subpopulations` turned on**: "SubPopulation 1" is used as a sample subpopulation name. It will be the actual name as specified by the user.

### Option 2 (TestStat): Expected Members of the Output List

| **Members**     | **Type**  | **Description**                                                                                  |
|----------------|--------------|---------------------------------------------------------------------------------|
| **TestStat**   | Numeric  | Value of appropriate test statistic on Wald ﴾Z﴿ scale. If `Subpopulations` is turned on, this is a named list of numeric of length `DesignParam$NumSubPops`, with test statistics for each subpopulation, e.g., `TestStat[“FullPopulation”]`, `TestStat[“SubPopulation 1”]`, etc.|
...
|**HR**|Numeric|Estimate of the hazard ratio. Required if `LookInfo$FutBdryScale = 6` (futility boundary scale is hazard ratio scale). Only applicable when `Statistical Design = Group Sequential or Group Sequential with Sample Size Re-Estimation`. If `Subpopulations` is turned on, this is a named list of numeric of length `DesignParam$NumSubPops`, with hazard ratios for each subpopulation, e.g., `HR[“FullPopulation”]`, `HR[“SubPopulation 1”]`, etc.|
...
| **Members**     | **Type**  | **Description**                                                                                  |
|----------------|--------------|---------------------------------------------------------------------------------|
| **Decision**   | Vector of Integer  | Vector of length `DesignParam$NumTreatments`, containing the boundary crossing decision for each treatment arm:<br>– `0`: No boundary crossed.<br>– `1`: Lower efficacy boundary crossed.<br>– `2`: Upper efficacy boundary crossed.<br>– `3`: Futility boundary crossed (only applicable when `Statistical Design = Group Sequential`).<br>– `4`: Equivalence boundary crossed (not available in East Horizon Explore).<br>You can use the functions `CyneRgy::GetDecisionString` and `CyneRgy::GetDecision` to get the decision value. See the template below for the correct usage. If `Subpopulations` is turned on, this is a named list of integer of length `DesignParam$NumSubPops`, with decisions for each subpopulation, e.g., `Decision[“FullPopulation”]`, `Decision[“SubPopulation 1”]`, etc.|
|**AnalysisTime**|Numeric|Optional. Estimate of analysis time. For interim analyses, equivalent to look time; for final analysis, equivalent to study duration.|
| **ErrorCode**   | Integer  | Optional. Can be used to handle errors in your script:<br>– `0`: No error.<br> – `Positive Integer`: Nonfatal error, the current simulation will be aborted, but the next simulation will proceed.<br>– `Negative Integer`: Fatal error, no further simulations will be attempted.                  |
...
| **Members**     | **Type**  | **Description**                                                                                  |
|----------------|--------------|---------------------------------------------------------------------------------|
| **TestStat**   | Vector of Numeric  | Vector of length `DesignParam$NumTreatments`, containing the value of appropriate test statistic on Wald ﴾Z﴿ scale for each treatment arm. If `Subpopulations` is turned on, this is a named list of numeric of length `DesignParam$NumSubPops`, with test statistics for each subpopulation, e.g., `TestStat[“FullPopulation”]`, `TestStat[“SubPopulation 1”]`, etc.|
...
|**HR**|Numeric|Estimate of the hazard ratio. Required if `LookInfo$FutBdryScale = 6` (futility boundary scale is hazard ratio scale). Only applicable when `Statistical Design = Group Sequential`. If `Subpopulations` is turned on, this is a named list of numeric of length `DesignParam$NumSubPops`, with test statistics for each subpopulation, e.g., `HR[“FullPopulation”]`, `HR[“SubPopulation 1”]`, etc.|
|**AnalysisTime**|Numeric|Optional. Estimate of analysis time. For interim analyses, equivalent to look time; for final analysis, equivalent to study duration.|
| **ErrorCode**   | Integer  | Optional. Can be used to handle errors in your script:<br>– `0`: No error.<br> – `Positive Integer`: Nonfatal error, the current simulation will be aborted, but the next simulation will proceed.<br>– `Negative Integer`: Fatal error, no further simulations will be attempted.                  |
...
- [Analyze.TimeToEvent.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.R) for 2-Arm
- [Analyze.TimeToEvent.MAMS.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.MAMS.R) for Multiple Arm
- [Analyze.TimeToEvent.SSR.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.SSR.R) for SSR
- [Analyze.TimeToEvent.Stratification.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.Stratification.R) for Stratification
- [Analyze.TimeToEvent.Stratification.Subpopulation.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.Stratification.Subpopulation.R) for Stratification with Subpopulations
...
2. [**Time-To-Event Weighted Conditional Power Futility Analysis**](TimeToEventConditionalPowerFutilityAnalysis.html)
    - [AnalyzeTTEWithConditionalPowerFutility.R](https://github.com/Cytel-Inc/CyneRgy/tree/main/inst/Examples/TimeToEventConditionalPowerFutilityAnalysis/R/AnalyzeTTEWithConditionalPowerFutility.R)
