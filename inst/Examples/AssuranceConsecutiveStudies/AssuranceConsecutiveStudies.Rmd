...
flowchart <- data.frame(
  ...
  label = c("Load prior distribution parameters,\nmean, and standard deviations", 
            "Sample treatment effect from\nmixture of normal distributions",
            "Compute treatment means\nincluding sampled effect",
...
flowchart2 <- data.frame(
  ...
  label = c("Load prior distribution parameters,\nSD, MAV and PU",
            ...
            "Make decision based on posterior\nprobability and user-defined cutoff",
            "Return posterior parameters,\nprobabilities, and decision"),
  fill = rep("#cfe2ff", 5)
)
# Define arrows for flowchart 
...
ymaxfifthbox2 = yminfourthbox2 - spaceybox
yminfifthbox2 = ymaxfifthbox2 - unitybox*1.5
ymaxsixthbox2 = yminfifthbox2 - spaceybox
yminsixthbox2 = ymaxsixthbox2 - unitybox*1.5
# Define flowchart steps 
flowchart2 <- data.frame(
  xmin = rep(bigcolxmin2 + 0.1, 6),
  xmax = rep(bigcolxmax2 - 0.1, 6),
  ymax = c(ymaxfirstbox, ymaxsecondbox2, ymaxthirdbox2, ymaxfourthbox2, ymaxfifthbox2, ymaxsixthbox2),
  ymin = c(yminfirstbox2, yminsecondbox2, yminthirdbox2, yminfourthbox2, yminfifthbox2, yminsixthbox2),
  label = c("Load prior distribution parameters,\nSD, MAV, PU, and PUFutility",
            "Compute posterior parameters",
            "If IA: simulate future data and\nestimate predictive prob. of futility",
            "If FA: sample posterior\ndistributions of each arm",
            "Make decision based on posterior\nprobability and user-defined cutoff",
            "Return posterior parameters,\nprobabilities, and decision"),
  fill = rep("#cfe2ff", 6)
)
# Define arrows for flowchart 
flowchart_arrows2 <- data.frame(
  x = rep((bigcolxmin2+bigcolxmax2)/2, 5),
  xend = rep((bigcolxmin2+bigcolxmax2)/2, 5),
  y = c(flowchart2$ymin[1], flowchart2$ymin[2], flowchart2$ymin[3], flowchart2$ymin[4], flowchart2$ymin[5]),
  yend = c(flowchart2$ymax[2], flowchart2$ymax[3], flowchart2$ymax[4], flowchart2$ymax[5], flowchart2$ymax[6])
)
# Define the legend elements
legend_data <- data.frame(
  xmin = c(9, 10.5),
...
p <- ggplot() +
  ...
  scale_fill_identity() +
  # Add labels for the columns
  geom_text(data = columns, aes(x = (xmin + xmax) / 2, y = ymax + 1, label = label), size = 3, angle = 20, vjust = 1) +
  # Add flowchart 
  geom_rect(data = flowchart, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = fill), color = "black") +
  geom_text(data = flowchart, aes(x = (xmin + xmax) / 2, y = (ymin + ymax) / 2, label = label), size = 2.5) +
  # Add arrows between flowchart boxes
