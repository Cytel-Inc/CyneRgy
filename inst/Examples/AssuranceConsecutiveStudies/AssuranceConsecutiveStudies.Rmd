...
This example considers a fixed sample design with normally distributed outcomes $Y$, with 80 patients per treatment arm. It demonstrates how to customize the **Analysis** endpoint to compute the probability of success, and the **Response (Patient Simulation)** endpoint to simulate a mixture of normal distributions.

By integrating an R function into the Analysis endpoint, users can evaluate the frequentist operating characteristics of the Bayesian design in East Horizon. Additionally, by modifying the Response endpoint, the simulation will incorporate Bayesian assurance. Specifically, an R function first samples from the assurance prior, then generates the patient data. The resulting power from this simulation reflects the Bayesian assurance, assuming the two-component prior.
