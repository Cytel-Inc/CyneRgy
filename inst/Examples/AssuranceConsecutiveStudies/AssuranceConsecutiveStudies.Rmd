...
In the [RCode directory of this example](https://github.com/Cytel-Inc/CyneRgy/tree/main/inst/Examples/AssuranceConsecutiveStudies/RCode) you will find the R files used in the examples.
...
Using the East workbook named Assurance.cywx with East version $>=$ 6.5.4 the Example 1 simulation can be used to obtain the results found in the next section.   After editing the simulation, on the User Define R Functions the Generate Response and Compute Test Statistic are both replaced with R code.   The Generate Response utilizes the SimulatePatientOutcomeNormalAssurance found in the file named [RCode/SimulatePatientOutcomeNormalAssurance.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/AssuranceConsecutiveStudies/RCode/SimulatePatientOutcomeNormalAssurance.R) and has variables as shown below. For the Compute Test Statistic the function name AnalyzeUsingBayesianNormals found in  [RCode/AnalyzeUsingBayesianNormals.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/AssuranceConsecutiveStudies/RCode/AnalyzeUsingBayesianNormals.R) with input shown below. 


![Input for generate response in East](Example1GenerateResponseInput.jpg)

![Input for test statistitic input in East](Example1TestStatisticInput.jpg)
...
![Input for generate response in East Horizon Explore](Example1GenerateResponseSolara.jpg)

![Input for test statistic in East Horizon Explore](Example1TestStatisticSolara.jpg)
...
Denote the data at the interim analysis by $X_1$ and data for patients enrolled after the IA by $X_2$. If the predictive probability of a No-Go at the final analysis is greater than $PU_{Futility} = 90$% then the trial is stopped for futility. Specifically, if $Pr( \text{End of Study No-Go} | X_1 ) > PU_{Futility} = 90\%$ then the trial is stopped for futility. The prediction formula becomes:
...
The initialize function InitializeSurvival is found in the file [RCode/SimulatePatientOutcomeNormalAssurance.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/AssuranceConsecutiveStudies/RCode/InitializeSurvival.R).   The function used for analysis is AnalyzeSurivalDataUsingCoxPH, is found in the file [RCode/AnalyzeSurvivalDataUsingCoxPH.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/AssuranceConsecutiveStudies/RCode/AnalyzeSurvivalDataUsingCoxPH.R)  The function used for simulating patient data is SimulatePatientOutcomeNormalAssurance and is found in the file [RCode/SimulatePatientOutcomeNormalAssurance.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/AssuranceConsecutiveStudies/RCode/SimulatePatientOutcomeNormalAssurance.R)
