...
`RunExample()` opens `ProbabilitySuccessDualEndpoints.Rproj` in RStudio. In VS Code it opens the example folder, `Description.Rmd`, and every R script
...
<p style="margin-bottom:0">**Note:** This example is compatible with both Fixed Sample and Group Sequential statistical designs. The R code automatically detects whether interim look information (*LookInfo*) is available and adjusts the analysis parameters accordingly. When using Group Sequential, the user parameter *HazardRatioCutoffIA* is mandatory.</p>
</div>

<div class="alert alert-warning" role="alert">
<p style="margin-bottom:0">**Important note:** This example does not use East Horizon's built-in dual-endpoint framework, as it was developed before that functionality existed.</p>
</div>

# Introduction

This example demonstrates how to compute the probability of success for a clinical trial by extending East Horizon’s single-endpoint framework to handle dual endpoints through custom R scripts for the **Response (Patient Simulation)** and the **Analysis** integration points. This example uses **Progression Free Survival (PFS)** and **Overall Survival (OS)** as endpoints, however, this could be extended to other types of endpoints by modifying the R code.
...
CyneRgy::PlotExampleFlowchart(
    lIntPoints = list(
        "Response" = c(
            "Load prior distribution parameters", 
            "Sample median time to PFS and OS from Gamma distribution",
            "Sample probability of death before PFS from Beta distribution",
            "Simulate PFS and OS using multi-state model", 
            "Return survival times"
        ),
        "Analysis" = c(
            "Load OS hazard ratio thresholds", 
            "Analyze PFS using Cox regression",
            "Analyze OS using Cox regression",
            "Make interim or final analysis decisions based on efficacy and futility boundaries", 
            "Return test statistics, hazard ratios, and decision outcomes"
        )
    )
)
```
