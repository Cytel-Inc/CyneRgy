...
Valeria test: This example demonstrates how to compute the probability of success of clinical trial by extending East Horizon’s single-endpoint framework to handle dual endpoints (Progression-Free Survival and Overall Survival) using custom R scripts for the **Analysis** and **Response (Patient Simulation)** endpoints. This example could be extended to other types of endpoints by modifying the R code. 
...
|---|------|
|**HazardRatioCutoffIA**|OS hazard ratio threshold for interim analysis|
|**HazardRatioCutoffFA**|OS hazard ratio threshold for final analysis|

## Option 1: Fixed Sample Design

The first option is a fixed sample design with a hazard ratio threshold of 0.9. It shows that we can still use this script without interim analyses. Refer to the table below for the values of the user-defined parameters used in this option.
...
As we don't have an interim analysis here (as we are using a fixed sample design), the first user parameter (*HazardRatioCutoffIA*) is skipped.
...
## Option 3: Low Marginal PoS for OS

The third option is a fixed sample design with a lower hazard ratio threshold of 0.5. Refer to the table below for the values of the user-defined parameters used in this option.
...
## Option 4: High Marginal PoS for OS

The fourth option is a fixed sample design with a higher hazard ratio threshold of 1.2. Refer to the table below for the values of the user-defined parameters used in this option.
...
We are using East Horizon’s single-endpoint framework, which we customize to support dual endpoints through the Response Endpoint via our R script. The two endpoints of interest are:

- **Progression-Free Survival (PFS)**: The duration during which a patient lives with the disease without experiencing further progression.
...
1. Direct death from initial state:
   - Occurs with probability $p$.
   - Survival time is $OS = X_2$ (time to death without progression).

2. Progression followed by death:
...
|**dProbOfDeathBeforeProgression0Param1**|Alpha parameter for probability of death before PFS for control group|
|**dProbOfDeathBeforeProgression0Param2**|Beta parameter for probability of death before PFS for control group|
|**dProbOfDeathBeforeProgression1Param1**|Alpha parameter for probability of death before PFS for treatment group|
|**dProbOfDeathBeforeProgression1Param2**|Beta parameter for probability of death before PFS for treatment group|
...
In the Results section, *Power* now refers to the probability of success, where success is defined as a statistically significant progression-free survival (PFS) and a positive trend in overall survival (OS). Below is an example of the heatmap that could be generated following the simulation.

![](ProbabilitySuccessDualEndpointsPFSOS_files/Results.png)
