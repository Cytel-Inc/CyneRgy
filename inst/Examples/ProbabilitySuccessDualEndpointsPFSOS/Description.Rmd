...
# Dummy plot
p <- plot_ly(x = seq(0, 50, length.out = 100), y = rep(0, 100), type = "scatter", mode = "lines") %>%
  layout(title = "Gamma Priors: Control vs Treatment", xaxis = list(title = "x"), yaxis = list(title = "Density"))
    ...
    script.onload = function() {
      ...
      form.innerHTML = `
        <div style='display: flex; gap: 20px; flex-wrap: wrap; font-family: sans-serif;'>
          <div>
            <label>Control Shape (dMedianPFS0PriorShape):<br>
              <input id='shape0' type='number' value='5' min='0.1' step='0.1' style='width:100px'>
            </label><br>
            <label>Control Rate (dMedianPFS0PriorRate):<br>
              <input id='rate0' type='number' value='1' min='0.01' step='0.1' style='width:100px'>
            </label>
          </div>
          <div>
            <label>Treatment Shape (dMedianPFS1PriorShape):<br>
              <input id='shape1' type='number' value='7' min='0.1' step='0.1' style='width:100px'>
            </label><br>
            <label>Treatment Rate (dMedianPFS1PriorRate):<br>
              <input id='rate1' type='number' value='1' min='0.01' step='0.1' style='width:100px'>
            </label>
          </div>
        </div>
        <br>
      ...
      const xs = Array.from({length: 400}, (_, i) => i * 0.25);

      function update() {
        const shape0 = parseFloat(document.getElementById('shape0').value);
        const rate0  = parseFloat(document.getElementById('rate0').value);
        const shape1 = parseFloat(document.getElementById('shape1').value);
        const rate1  = parseFloat(document.getElementById('rate1').value);

        const ys0 = xs.map(x => jStat.gamma.pdf(x, shape0, 1 / rate0));
        const ys1 = xs.map(x => jStat.gamma.pdf(x, shape1, 1 / rate1));
        ...
        Plotly.react(el, [
          ...
          title: 'Gamma Priors for Median PFS',
          xaxis: {title: 'x'},
          yaxis: {title: 'Density'},
          legend: {x: 0.7, y: 1}
        });
      ...
      // Add listeners
      ['shape0', 'rate0', 'shape1', 'rate1'].forEach(id => {
