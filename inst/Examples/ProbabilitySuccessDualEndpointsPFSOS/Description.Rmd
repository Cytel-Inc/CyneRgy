...
Thus, OS is X2 with probability p and X1 + X3 and with probability q.   We also know that the mean OS is $M_{OS}$ since the user provides it and thus could solve for $\alpha_{12}$ numerically given $\alpha_{01}$ and $\alpha_{02}$.   This approach would require using a root finding function, such as uniroot, to compute $\alpha_{12}$ .  
