...
**East Workbook**: GeneragePoissonArrival.cywx
...
Suppose that the expected accrual rates where 3, 6, 10, 15, 18, 25, 35 starting at time unit 0, 1, 2, 3, 4, 5, 6.  One example trial could have accrual that looks like the following:
...
In this example we make use of the build in inputs for East and Solara to provide  a ramp-up in accrual where the user can specify

Suppose that the user specified the start periods as 0, 6, 12, 18, 24, 48 and the expected accrual rates of 1, 4, 12, 24, 48.  Then from time 0 to time 6 the expected rate is 1 patient per time unit, from time 6 to 12 the rate is 4 patients per time unit, time 12 to 18 expected accrual is 12, 18 to 24 the rate is 24 to 24 and after time unit 24 it is assume to be 48. One example trial could have accrual that looks like the following, with dashed lines at the time points where accrual rates change:
