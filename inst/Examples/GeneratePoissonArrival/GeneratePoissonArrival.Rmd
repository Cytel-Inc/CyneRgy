...
This example does not use any user-defined parameters, but uses built-in parameters specified in East Horizon. We use the built-in inputs of start periods and expected accrual rates to implement a user-specified ramp-up in accrual using Poisson. Refer to the table below for more information.
...
# Define flowchart steps inside "Dropout"
...
# Define arrows for flowchart inside "Dropout"
...
Now, let's extend this approach by incorporating custom user-defined variables (*UserParam*). In this example, we use the same R function as before but we specify seven accrual rates as user parameters in East Horizon after selecting the R script. When custom variables are provided, the built-in variables *PrdStart* and *AccrRate* are ignored. Here, we cannot specify the start of the periods. Refer to the table below for the definitions and values of the user-defined parameters used in this example.
