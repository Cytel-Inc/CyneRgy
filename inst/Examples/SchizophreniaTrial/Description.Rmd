...
<div class="alert alert-primary" role="alert">
  ...
  <p style="margin-bottom:0">
    ...
    IntegrationPointResponse.html<strong>Integration Point: Response</strong></a>

    and the

    IntegrationPointAnalysis.html<strong>Integration Point: Analysis</strong></a>.

    Click on the links for more information about these integration points.
...
**RStudio Project File**: [SchizophreniaTrial.Rproj](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/SchizophreniaTrial.Rproj)
...
[AnalyzeMMRM.R](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/R/AnalyzeMMRM.R) -  This code takes simulated patient data and runs MMRM analysis.

[AnalyzeMMRM_GLS.R](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/R/AnalyzeMMRM_GLS.R) -  This code is a slower version of the original analysis, but it has slightly increased power and an alternative form of analysis using the function for generalized least squares in the R package nlme.
...
[PlotPatientData.R](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/R/PlotPatientData.R) -  This code takes the generated patient data and creates plots of individual patient trajectories.
...
For East Horizon, you will need **AnalyzeMMRM.R** and/or **AnalyzeMMRM_GLS.R** for the MMRM analysis.  Both files provide MMRM analysis but use different R packages and the **AnalyzeMMRM.R file** has the most efficient approach.  Additionally, the **GenerateMMRMResponses.R** file provides a R function for patient generation that can be used in East Horizon, but it is not required as the native approach to data generation could be used.


## Example outputs 

Below are some graphs as example outputs from the code from the **GenerateMMRMResponse** function.  It is always good to review example data sets to verify that the data is realistic. 

 <br><br><br>
<span style="font-size:13pt">**Figure 1**: An example plot of the trial data with a 95% CI</span>

![](Schizophrenia_Files/TrialPlot.png){width=90%}

 <br><br><br>
<span style="font-size:13pt">**Figure 2**: An example plot of individual patient trajectories</span>
...
## Input Setup
To recreate this example in East Horizon, begin by creating a new project in East Horizon using a **Study Objective** of "Two Arm Confirmatory." On the Plan page, select the Continuous endpoint type and choose the "Repeated Measure" option. Lastly, click the "Add Visit" so that the project will have a total of 5 visits: 4 interim visits and 1 final visit. 
...
For Better Response, select "Smaller Value," since a patient's outcome is improved with a smaller score. The project setup should look like the following.

<span style="font-size:13pt">**Project Setup**:</span>
![](Schizophrenia_Files/ProjectSetup.png){width=90%}

Once you "Create Project", you then need to create an input set using the explore option. 

## Design Page

In the MMRM fixed-sample card, the sample size should be 266. The group sequential MMRM card should have the input "200:360:20." This will create samples ranging from 200-360 patients counting up in steps of 20. both cards use a Type I error 0f 0.025 and an allocation ration of 1. For the Test type, select User Defined R, the upload the 
...
<span style="font-size:13pt">**MMRM Fixed Sample**:</span>
![](Schizophrenia_Files/MMRMFixed.png){width=90%}
 ...
 <br><br>
<span style="font-size:13pt">**MMRM Group Sequential**:</span>
...
<span style="font-size:13pt">**Response Card using data from the publication**:</span>
![](Schizophrenia_Files/PublicationResponse.png){width=90%}
<br><br><br>

<span style="font-size:13pt">**Null Case Card**:</span>

![](Schizophrenia_Files/NullResponse.png){width=90%}
<br><br><br>

<span style="font-size:13pt">**Cards with mean difference 4 and 8**:</span>
![](Schizophrenia_Files/MeanDiff4.png){width=90%}
![](Schizophrenia_Files/Diff8Response.png){width=90%}
<br><br><br>

<span style="font-size:13pt">**Cards with SD 10 and 20**:</span>
...
All dropout cards share the same dropout rates as the publication (0.26 control and 0.15 experimental). 

**Here is what the dropout card should look like**:
<br><br>

<span style="font-size:13pt">**Dropout Information**:</span>
...
<span style="font-size:13pt">**Enrollment Card**:</span>
...
<span style="font-size:13pt">**Simulation Setup**:</span>
![](Schizophrenia_Files/SimulationSetup.png){width=90%}
<br><br><br>
...
<span style="font-size:13pt">**Figure 3**: Simulation for Publication Results</span>
...
<span style="font-size:13pt">**Figure 4**: Difference of Means Results</span>
![](Schizophrenia_Files/MeanDiff4_8Results.png)
<br><br><br>

<span style="font-size:13pt">**Figure 5**: Standard Deviation Results</span>

![](Schizophrenia_Files/SD10_20Results.png)
<br><br><br>

- **Null behavior (MMRM)**: Type I error was observed between 2.1–3.6%, close to nominal 2.5%.

<br><br><br>

<span style="font-size:13pt">**Figure 6**: Null Case Results</span>
...
-  **Fixed Vs. selected Sample GS300**: While the Selected Sample GS300 (i.e., the group sequential design that allows for interim analyses and potential early stopping) does feature a lower power than the fixed sample, it does come with several benefits worth weight when simulating the study. Selected Sample GS300 boasts a 30% chance of stopping early, a shorter average study duration, and smaller average sample size. In return, it comes with a decrease in power by ~1.4%. These difference are significant as it can save money in a trial for even one of these other improved results. These designs were considered over larger sample sizes due to meeting the required 90% power with the smallest sample sizes.
![](Schizophrenia_Files/SelectDesign.png)
<br><br><br>

-  **Sensitivity Analysis**: The reason for including the difference of means cards and the standard deviation cards was to illustrate how sensitive the power in this study is if the researchers' initial assumptions were off. When increasing standard deviation to 20 or decreasing the difference of means to 4 led to the power becoming under powered. This means we would have to increase the sample size to detect if there is a significant difference. When the standard deviation was set to 10 and the difference between means was 8, the power increased, reaching the mid-to-high 90% range. This would allow for a reduction in the study's sample size while still maintaining 90% power.
