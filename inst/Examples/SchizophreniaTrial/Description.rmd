...
[RunSimulation.R](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/RCode/RunSimulation.R) -  This code allows for the code to be run locally, change variables around, and visualize data.  It can helpful to run the R code locally before uploading to EH to make sure no bugs are present if changes are made to the code. 

This example looks into an analysis using MMRM. It allows for dropout information to not impact power as much has other analysis methods. It works well in East Horizon (EH) for providing a longitudinal analysis design.  While not all of the R files listed above are required, they can be used to help simulate patient data and run analysis locally in R Studio.  This can be very helpful to allow users a good starting point in case they need to adjust the details in the analysis.  The various plot functions can be very helpful for visualizing the simulated patient data to make sure it looks realistic. 

For EH, you will need AnalyzeMMRM.R and/or AnalyzeMMRM_GLS.R for the MMRM analysis.  Both files provide MMRM analysis but use different R packages and the AnalyzeMMRM.R file has the most efficient approach.  Addionaly, the GnerateMMRResponse.R file provides a R function for patient generation that can be used in EH, but it is not required as the native approch to data generation could be used.
...
To recreate this example in EH, begin by creating a new project in EH using the Continuous endpoint type. Next create an input set using the explore option.  When setting up the project details, make sure to select the option to have repeated measurements and specify the times of the visits. 

## Designs: 

MMRM fixed‑sample (baseline N ~266) and group‑sequential MMRM in the sample size box insert 200:360:20 (this will create sample sizes ranging from 200 to 360 in steps of 20) (both at Type I error 0.025 and an allocation ratio of 1). For the Test type, select User Defined R, then upload the  [AnalyzeMMRM.R](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/RCode/AnalyzeMMRM.R).  
...
All cards use the R integration and the [GenerateMMRMResponses.R](https://github.com/Cytel-Inc/CyneRgy/blob/304-create-mmrm-analysis-example-for-clinical-trial/inst/Examples/SchizophreniaTrial/RCode/GenerateMMRMResponses.R) file can be uploaded..
...
## Dropout & Enrollment:
All dropout cards share the same dropout rates as the publication (0.26 control and 0.15 experimental). Next, there should be one enrollment card with a rate of 30 subjects per month. Once all of this are setup, go to simulation setup and select the 1000 repetitions.
