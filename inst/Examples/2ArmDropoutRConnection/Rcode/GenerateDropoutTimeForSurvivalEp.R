...
#'@name GenDropoutTime
#'@Endpoints : 2 arm Survival Design
...
#'@param TreatmentID : Array specifying indexes of arms to which subjects are allocated (one arm index per subject). Index for placebo / control is 0. This argument is passed from Engine.
...
#'@param PrdTime: Array of times used to specify dropout parameters.
...
#' @return retval : An array of generated drop out times
#'


GenDropoutTimeForSurvival <- function( NumSub, NumArm, TreatmentID, DropMethod, NumPrd, PrdTime, DropParam, UserParam = NULL )
{
  ...
  if( DropMethod == 1 )    # Dropout Hazard Rates
  {
    ...
    {
      retval[ vIndexControl[ 1:length( vIndexControl ) ] ]           <-   rexp( length( vIndexControl ), rate = DropParam[ 1 ] )
    }
    if( DropParam[ 2 ] > 0 )             # generate dropout time only in case of Non - zero dropout probability
    {
      retval[ vIndexExperiment[ 1:length( vIndexExperiment ) ] ]     <-   rexp( length( vIndexExperiment ), rate = DropParam[ 2 ] )
    }
  }	
  ...
  if( DropMethod == 2 )   # Probability of Dropout
  {
    ...
    if( DropParam[ 1 ] > 0 )            # generate dropout time only in case of Non - zero dropout probability
    {
      retval[ vIndexControl[ 1:length( vIndexControl ) ] ]           <-   rexp( length( vIndexControl ), rate = dExpDropoutControlRate )
    }
    if( DropParam[ 2 ] > 0 )           # generate dropout time only in case of Non - zero dropout probability
    {
      retval[ vIndexExperiment[ 1:length( vIndexExperiment ) ] ]     <-   rexp( length( vIndexExperiment ), rate = dExpDropoutExperimentRate)
    }
    
  }
  
  return( list( DropOutTime = as.double( retval ), ErrorCode = as.integer( Error ) ) );
}