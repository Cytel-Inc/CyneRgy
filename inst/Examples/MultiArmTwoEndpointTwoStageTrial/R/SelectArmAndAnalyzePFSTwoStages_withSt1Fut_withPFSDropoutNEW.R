...
#'          \item{DropoutProportion}{Drop-out proportion in PFS follow-up}
...
SelectArmAndAnalyzePFSTwoStages <- function( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
{
    ...
    if ( !is.null( UserParam$DropoutProportion ) && UserParam$DropoutProportion > 0 )
    {
        ...
        if ( nDropouts > 0 )
        {
            # Randomly select which patients drop out
            vDropoutIndices <- sample( seq_len( nrow( dfSimDataStage2 ) ), nDropouts )
            
            # Generate dropout times for selected patients
            dfSimDataStage2$DropoutTime <- Inf
            
            # Use a  controlled maximum dropout time, for example
            dMaxDropoutTime <- quantile( dfSimDataStage2$PFSNonCens, 0.90, na.rm = TRUE )
            
            dfSimDataStage2$DropoutTime[ vDropoutIndices ] <- runif( nDropouts, 
                                                                     min = 0, 
                                                                     max = dMaxDropoutTime )
        } else {
            dfSimDataStage2$DropoutTime <- Inf
