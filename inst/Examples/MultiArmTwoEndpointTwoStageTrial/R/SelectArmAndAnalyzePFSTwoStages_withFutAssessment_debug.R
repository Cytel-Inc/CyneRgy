...
SelectArmAndAnalyzePFSTwoStages <- function( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
{
    ...
    vPatientOutcome   <- SimData$Response[ 1:nStage1NumCompleters ]
    vPatientTreatment <- SimData$TreatmentID[ 1:nStage1NumCompleters ]
    ...
    {
        # Genrate Dropout Time per patient
    ...
    original_nStage2NumPatients <- nrow(dfSimDataStage2)
    dfSimDataStage2 <- dfSimDataStage2[ dfSimDataStage2$ArrivalTime <= dStage2AnalysisTiming ,]
    filtered_nStage2NumPatients <- nrow( dfSimDataStage2 )
    ...
    # Code below from AnalyzeUsingEastLogrankFormula.R.
    ...
    lRet <- ReturnResult( nTrtArms,
                          ...
                          original_nStage2NumPatients,
                          filtered_nStage2NumPatients)
...
ReturnResult <- function( nTrtArms,    
                          ...
                          nStage2NumPatients = NA_real_,
                          dStage2AnalysisTiming = NA_real_,
                          original_nStage2NumPatients = NA_real_,
                          filtered_nStage2NumPatients = NA_real_)
{
    ...
    for ( i in 1:nTrtArms ) 
    {
        ...
        lRet[[paste0("Stage2AnalysisTiming_", i)]] <- as.double(dStage2AnalysisTiming)
        lRet[[paste0("original_nStage2NumPatients", i)]] <- as.double(original_nStage2NumPatients)
        lRet[[paste0("filtered_nStage2NumPatients", i)]] <- as.double(filtered_nStage2NumPatients)
