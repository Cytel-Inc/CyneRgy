...
<p style="margin-bottom:0">This example is related to the <a href="IntegrationPointResponseContinuous.html" class="alert-link">**Integration Point: Response - Continuous Outcome**</a>. Click the link for setup instructions, variable details, and additional information about this integration point.</p></div>
...
Important: **ArrivalTime** is a new required parameter. Existing R scripts must be updated to include this parameter in the function definition, even if it is not used. See <a href="IntegrationPointResponseContinuous.html" class="alert-link">**Integration Point: Response - Continuous Outcome**</a> for more information.
</p></div>
...
**East Workbook**: [2ArmNormalOutcomePatientSimulation.cywx](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/2ArmNormalOutcomePatientSimulation/2ArmNormalOutcomePatientSimulation.cywx)
...
CyneRgy::PlotExampleFlowchart(
    lIntPoints = list(
        "Response" = c(
            "Load probabilities of non-response (dProbOfZeroOutcomeCtrl and dProbOfZeroOutcomeExp)", 
            "Loop through patients", 
            "Simulate whether response is null using binomial distribution", 
            "If not null, simulate response using normal distribution", 
            "Return responses"
        )
    )
)
...
This demonstrates that using the R function with a non-responder probability of zero produces the same results as simulating the data directly with the default engine of East Horizon .

## Example 1.2 - 20% of Non-Responders

Now, assume that, on average, 20% of patients will not respond to treatment. A binomial distribution is used to determine whether a patient is a non-responder. If the patient is a responder, their outcome is simulated from a normal distribution using the mean and standard deviation specified in East Horizon and sent to R. Refer to the table below for the values of the user-defined parameters used in this example.
...
CyneRgy::PlotExampleFlowchart(
    lIntPoints = list(
        "Response" = c(
            ...
            "Simulate the probabilities of non-response using Beta distribution",
            "Loop through patients", 
            "Simulate whether response is null using binomial distribution", 
            "If not null, simulate response using normal distribution", 
            "Return responses"
