...
In this example, the coxph() from "survival" package in R is utilized to analyze the data and compute at a user-specified confidence interval. The team would like to make a Go decision if there is at least a 90% chance that the Hazard Ratio is less than the MAV.  If a Go decision is not made, then a No Go decision is made if there is less than a 10% chance the Hazard ratio is less than the TV.  Using a frequentist CI an approximation to this design can be done by the logic described below.

#At an Interim Analysis
    If the Lower Limit of the CI, denoted by LL, is less than a log of user-specified amount then a Go decision is made.  Specifically, if LL < log user-specified lower limit --> Go
    If a Go decision is not made, then if the Upper Limit of the CI, denoted by UL, is greater than a log of user-specified value a No Go decision is made.  Specifically, if UL > log user-specified upper limit --> No Go
    Otherwise, continue to the next analysis.

#At the Final Analysis: 
    If the Lower Limit of the CI, denoted by LL, is less than than a log of user-specified values then a Go decision is made.  Specifically, if LL < log user-specified value --> Go
