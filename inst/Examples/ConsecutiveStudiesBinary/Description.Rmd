...
This example demonstrates the computation of conditional probability of success in consecutive studies through the integration of R with Cytel products by using custom R scripts for the **Response (Patient Simulation)** and the **Analysis** integration points. The examples feature a sequential design involving a Phase 2 trial followed by a Phase 3 trial, both with a binary endpoint. Specifically, we compute the conditional assurance of Phase 3 given success in Phase 2.
...
- **[AnalyzeUsingEastManualFormula.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Examples/ConsecutiveStudiesBinary/R/AnalyzeUsingEastManualFormula.R)**
   ...
   This R file is used for the **Analysis** integration point of both Phase 2 and Phase 3 studies. It contains a function that performs the analysis based on formulas of the **Chapter 24: Binomial Superiority Two‐Sample** from the East manual (24.2).
...
15. Specify the number of simulation runs as needed. You can choose any number you require.
...
24. Open the downloaded file in Excel. Filter the Decision column (column F) to only show rows with “Efficacy”.
...
27. Click on the input set you just created, then select “User Specified – R” in the Test field. Like for the Phase 2 study, select the file **AnalyzeUsingEastManualFormula.R**, and click on the “Save” button to exit the R Integration details window. See Steps 3 to 7 for more information.
...
Using the file above, the Response element of the Phase 2 study is customized to generate patient outcomes for a binary response trial while incorportaing uncertainty about the true response rates by sampling them from prior distributions. Here, we use a **Beta distribution** prior. However, you could use any other distribution by changing the R code to sample from a different one on lines 37 and 38 of the file. 

For each arm, the true response probability is drawn from a Beta distribution:

$$
p_\text{ctrl} \sim \text{Beta}(\alpha_\text{ctrl}, \beta_\text{ctrl})
$$
$$
p_\text{exp} \sim \text{Beta}(\alpha_\text{exp}, \beta_\text{exp})
$$
...
$$
Y_i \sim \text{Bernoulli}(p_j)
$$

where $Y_i \in \{0,1\}$ is the binary response, and $p_j$ is the true response probability ($p_\text{ctrl}$ or $p_\text{exp}$ depending whether the patient $i$ is in the control or experimental arm).
...
Using the file above, the Response element of the Phase 3 study is customized. Here, instead of sampling from a Beta distribution, we use the true response rates informed by the Phase 2 study. No user parameter is used for this file. 


The *LoadData* function is used to load the results of the Phase 2 study. See Steps 24 and 25 of the [Step-by-Step Instructions](#step-by-step-instructions) section for more information about how to load your own results.
...
For both Phase 2 and Phase 3 studies of this example, the analysis is based on formulas of the **Chapter 24: Binomial Superiority Two‐Sample** from the East manual (24.2). No user parameter is used for this integration point.
