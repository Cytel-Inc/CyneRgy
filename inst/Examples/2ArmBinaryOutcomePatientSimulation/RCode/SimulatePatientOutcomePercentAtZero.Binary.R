...
#' @title Simulate patient outcomes from a binary distribution with a percent of patients having an outcome of 0. 
...
#' simulated from a binary distribution with the mean and standard deviation as sent from East. 
SimulatePatientOutcomePercentAtZero.Binary <- function(NumSub, NumArm, TreatmentID, PropResp,UserParam = NULL)
{
    ...
    for( nPatIndx in 1:NumSub )
    {
        ...
        if( nResponseIsZero == 0  )  # The patient responded, so we need to simulate their outcome from a binary distribution 
