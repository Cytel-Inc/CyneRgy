...
2.  AnalyzeUsingZTestNormal.R - Contains a function named AnalyzeUsingZTestNormal to demonstrate the R code necessary for this example as described below.

3. LibraryBSDA.R - Contains a function named LoadBSDA to load the package named "BSDA" required for carrying out Z.test() in R.
...
## Example 2 - Two-arm Arm Normal Analysis Using the z.test function in R  

This example utilizes the z.test() function in R to perform the analysis. The p-value from z.test is used to compute the Z statistic that is compared to the upper boundary computed and sent by East as an input.  This example does NOT include a futility rule. 
