...
#'          SimData$ResponseX   :   Numeric value (0:Non-responder/1:Responder) for the binary response.
...
AnalyzeUsingFisherExactTest <- function(SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
{
    ...
    Response <- SimData[[paste0("Response", EPID)]]
    vPatientOutcome      <- Response[1:nQtyOfTargets]
    vPatientTreatment    <- SimData$TreatmentID[1:nQtyOfTargets]
    
    # Create contingency table for Fisher's Exact Test
    mContingencyTable    <- table(vPatientTreatment, vPatientOutcome)
    
    # Perform Fisher's Exact Test
    if(DesignParam$TailType[[EPID]] == 0) 
    {
