...
#' @name {{FUNCTION_NAME}}
#' @title Analyze Stratified Time-to-Event Outcomes
#' @description Analyze simulated time-to-event outcomes using the configured stratification factors.
...
#'                      }
#' This template preserves the stratified analysis function signature expected by East Horizon.
######################################################################################################################## .
...
{{FUNCTION_NAME}} <- function( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
{
    ...
    # error appropriately.
    return( list( Decision = as.integer( retval ), AnalysisTime = as.double( EstAnalysisTime ), ErrorCode = as.integer( nError ) ) )
}
