...
#' @param SurvMethod This value is pulled from the Input Method drop-down list. It will be 1 (Hazard Rate), 2 (Cumulative % survival), or 3 (Medians).
...
#'      \item{If SurvMethod = 2}{SurvParam is an array (NumPrd rows, NumArm columns) that specifies arm-by-arm cumulative % survivals 
#'      (one value per arm per piece). Thus, SurvParam[i, j] specifies the cumulative % survival in the ith period for the jth arm.}
