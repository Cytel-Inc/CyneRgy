...
#' @description Use the Z.test() function inside "BSDA" library to compute the statistic.  The purpose of this example is to demonstrate how the analysis and decision making can be modified in a simple approach.  
...
AnalyzeUsingZTestNormal <- function( SimData, DesignParam, LookInfo, UserParam = NULL )
{   
       # Input objects can be saved through the following lines:
    
    # setwd( "D:\\Project\\CyneRgy\\CyneRgy\\Normal_rds")
    # saveRDS( SimData, "SimData.Rds")
    # saveRDS( DesignParam, "DesignParam.Rds" )
    # saveRDS( LookInfo, "LookInfo.Rds" )
    ...
    lAnalysisResult           <- z.test( vOutcomesE, vOutcomesS, alternative = "greater", sigma.x = dStdDevOfResponsesOnE, 
                                   sigma.y = dStdDevOfResponsesOnS )
    ...
    lRet <- list( TestStat = as.double( dZValue ),
