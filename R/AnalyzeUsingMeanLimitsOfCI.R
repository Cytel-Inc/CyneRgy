...
#' \describe{
#'   \item{UserParam$dLowerLimit}{A value (-Inf, Inf) that specifics the lower limit, eg  Minimum Acceptable Value (MAV). }
#'   \item{UserParam$dUpperLimit}{A value (-Inf, Inf) that specifies the upper limit for the confidence interval, eg Target Value (TV).}
...
#'               In this example, the t.test() from base package in R is utilized to analyze the data and compute at user-specified confidence interval (dConfLevel).  
...
AnalyzeUsingMeanLimitsOfCI <- function(SimData, DesignParam, LookInfo = NULL, UserParam = NULL)
{
    ...
    # setwd( ["Path where we want .Rds files from East to be saved"])
    # saveRDS( SimData, "SimData.Rds")
    # saveRDS( DesignParam, "DesignParam.Rds" )
    # saveRDS( LookInfo, "LookInfo.Rds" )
    ...
    if( nDecision == 0 )
    {
        ...
        }else if( dUpperLimitCI < UserParam$dUpperLimit )     # At the IA check the No Go since a Go decision was not made
            nDecision <- 3 # East code for futility 
    }
