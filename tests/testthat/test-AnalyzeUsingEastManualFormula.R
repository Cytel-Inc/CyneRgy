...
#   Author: Author Name
#   Description: Test file for AnalyzeUsingEastManualFormula
#   Change History:
#   Last Modified Date: 01/02/2024
#################################################################################################### .


context( "AnalyzeUsingEastManualFormula")

test_that("Test- AnalyzeUsingEastManualFormula", {
    ...
    lRet1 <- tryCatch({
        AnalyzeUsingEastManualFormula( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet2 <- tryCatch({
        AnalyzeUsingEastManualFormula( SimData, DesignParam, LookInfo = LookInfo, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet3 <- tryCatch({
        AnalyzeUsingEastManualFormula( SimData, DesignParam )
    }, error = function(e) {
        NULL
    })
