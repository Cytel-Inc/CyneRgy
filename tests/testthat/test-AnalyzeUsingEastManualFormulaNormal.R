...
context( "AnalyzeUsingEastManualFormulaNormal")

test_that("Test- AnalyzeUsingEastManualFormulaNormal", {
    ...
    lRet1 <- tryCatch({
        AnalyzeUsingEastManualFormulaNormal( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet2 <- tryCatch({
        AnalyzeUsingEastManualFormulaNormal( SimData, DesignParam, LookInfo = LookInfo, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet3 <- tryCatch({
        AnalyzeUsingEastManualFormulaNormal( SimData, DesignParam )
    }, error = function(e) {
        NULL
    })
