...
#   Author: Author Name
#   Description: Test file for AnalyzeUsingPropLimitsOfCI
#   Change History:
#   Last Modified Date: 01/02/2024
#################################################################################################### .


context( "AnalyzeUsingPropLimitsOfCI")

test_that("Test- AnalyzeUsingPropLimitsOfCI", {
    ...
    lRet1 <- tryCatch({
        AnalyzeUsingPropLimitsOfCI( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet2 <- tryCatch({
        AnalyzeUsingPropLimitsOfCI( SimData, DesignParam, LookInfo = LookInfo, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet3 <- tryCatch({
        AnalyzeUsingPropLimitsOfCI( SimData, DesignParam )
    }, error = function(e) {
        NULL
    })
