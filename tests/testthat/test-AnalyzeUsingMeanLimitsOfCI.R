...
context( "AnalyzeUsingMeanLimitsOfCI")

test_that("Test- AnalyzeUsingMeanLimitsOfCI", {
    ...
    lRet1 <- tryCatch({
        AnalyzeUsingMeanLimitsOfCI( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet2 <- tryCatch({
        AnalyzeUsingMeanLimitsOfCI( SimData, DesignParam, LookInfo = LookInfo, UserParam = NULL )
    }, error = function(e) {
        NULL
    })
    ...
    lRet3 <- tryCatch({
        AnalyzeUsingMeanLimitsOfCI( SimData, DesignParam )
    }, error = function(e) {
        NULL
    })
