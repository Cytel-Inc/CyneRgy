...
``` r
CyneRgy::RunExample( "DEPDecisionsUsingMCP" )
```
...
``` r
CyneRgy::RunExample( "DEPDecisionsUsingMCP", strDirectory = getwd() )
```

[`RunExample()`](https://Cytel-Inc.github.io/CyneRgy/reference/RunExample.md)
opens `DEPDecisionsUsingMCP.Rproj` in RStudio. In VS Code it opens the
