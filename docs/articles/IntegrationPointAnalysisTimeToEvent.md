...
| **Type** | **Description** |
|----|----|
| List | A named list containing `ErrorCode` and one or multiple of the following members: `Decision`, `TestStat`, `AnalysisTime`, `Delta`, `CtrlCompleters`, `TrmtCompleters`, `CtrlPi`, `HR`, `StdError`, `ReEstEvents`, `ReEstCompleters`. See below for more information. |
...
*ReEstEvents* is required. Only Option 1 is supported, all other output
options are not available.

**Note for `Subpopulations` turned on**: “SubPopulation 1” is used as a
sample subpopulation name. It will be the actual name as specified by
the user.
...
- If `LookInfo$FutBdryScale = 3` (futility boundary scale is conditional
  ...
  `TestStat` will be used to check for efficacy and `Delta`,
  `CtrlCompleters`, `TrmtCompleters`, and `CtrlPi` will be used to check
  for futility.
- If `LookInfo$FutBdryScale = 6` (futility boundary scale is hazard
  ratio scale), `TestStat` will be used to check for efficacy and `HR`
...
`TestStat` will be used to perform multiplicity adjustment.
...
- [Analyze.TimeToEvent.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.R)
  for 2-Arm
- [Analyze.TimeToEvent.MAMS.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.MAMS.R)
  for Multiple Arm
- [Analyze.TimeToEvent.SSR.R](https://github.com/Cytel-Inc/CyneRgy/blob/main/inst/Templates/Analyze.TimeToEvent.SSR.R)
  for SSR
...
##### For `Statistical Design = Group Sequential with Sample Size Re-Estimation`
    ...
    PerformDecision <- function( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    {
        ...
        nReEstEvents  <- 0 # Initialize number of completers
        # Write the actual code here.
        
        return( list ( Decision = as.integer( nDecision ), ReEstEvents = as.integer( nReEstEvents ), ErrorCode = as.integer( nError ) ) )
    }

##### For `Subpopulations` turned on

    PerformDecision <- function( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    {
        nError                          <- 0 # Error handling (no error)
        
        lDecision                       <- list() # Initialize decision list
        lDecision[["FullPopulation"]]   <- 0 # Initialize decision for full population
        lDecision[["SubPopulation 1"]]  <- 0 # Initialize decision for subpopulation 1 (sample name, should be changed with the one specified in EH)
        # Write the actual code here.
        
        return( list ( Decision = as.list( lDecision ), ErrorCode = as.integer( nError ) ) )
    }
...
##### For `Subpopulations` turned on
    ...
    PerformDecision <- function( SimData, DesignParam, LookInfo = NULL, UserParam = NULL )
    {
        ...
        lTestStat                       <- list() # Initialize test statistic list
        lTestStat[["FullPopulation"]]   <- 0 # Initialize test statistic for full population
        lTestStat[["SubPopulation 1"]]  <- 0 # Initialize test statistic for subpopulation 1 (sample name, should be changed with the one specified in EH)
        # Write the actual code here.
        
        return( list ( TestStat = as.list( lTestStat ), ErrorCode = as.integer( nError ) ) )
    }
...
##### For `Subpopulations` turned on
    ...
    PerformDecision <- function( SimData, DesignParam, LookInfo, UserParam = NULL )
    {
        nError                          <- 0 # Error handling (no error)
