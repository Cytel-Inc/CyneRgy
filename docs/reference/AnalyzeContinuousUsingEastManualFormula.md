# Analyze Continuous Data Using East Manual Formula

Compute the test statistic using formula Q.3.3 in the East manual. This
example demonstrates how analysis and decision-making can be modified in
a simple approach. The test statistic is compared to the upper boundary
computed and sent by East or East Horizon as an input. Note that this
example does not include a futility rule. It performs a two-sample Z
test for a normal distribution with the assumption that the number of
looks is greater than one.

## Usage

``` r
AnalyzeContinuousUsingEastManualFormula(
  SimData,
  DesignParam,
  LookInfo = NULL,
  UserParam = NULL
